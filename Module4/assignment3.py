...
#
# .. your code here ..
df=pd.read_csv('D:\Python_Microscoft\DAT210x\Module4\Datasets\kidney_disease.csv',index_col=0, header=0)
...
labels = ['red' if i=='ckd' else 'green' for i in df2.classification]
...
#df3=df2.loc[:, ['bgr', 'wc', 'rc']]
#df3=df2.drop(labels=[ 'classification', 'rbc', 'pc', 'pcc', 'ba', 'htn', 'dm', 'cad', 'appet', 'pe', 'ane'], axis=1)



df3=df2.drop(labels=[ 'classification'], axis=1)
df3=pd.get_dummies(df2, columns=[ 'classification', 'rbc', 'pc', 'pcc', 'ba', 'htn', 'dm', 'cad', 'appet', 'pe', 'ane'])
...
df3.dtypes
df3.wc=pd.to_numeric(df3.wc)
df3.pcv=pd.to_numeric(df3.pcv)
df3.rc=pd.to_numeric(df3.rc)
