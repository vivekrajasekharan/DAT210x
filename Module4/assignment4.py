...
mat = scipy.io.loadmat('D:/Python_Microscoft/DAT210x/Module4/Datasets/face_data.mat')
...
iso.fit(df)
T1=iso.transform(df)
Plot2D(T1, "ISO", 0, 1, num_to_plot=40)
...
#ax.set_title("3d")
#ax.set_xlabel('Component: {0}'.format(T[:,0]))
#ax.set_ylabel('Component: {0}'.format(T[:,1]))
#ax.set_zlabel('Component: {0}'.format(T[:,2]))
#x_size = (max(T[:,0]) - min(T[:,0])) * 0.08
#y_size = (max(T[:,1]) - min(T[:,1])) * 0.08
#y_size = (max(T[:,2]) - min(T[:,2])) * 0.08
#for i in range(40):
#    img_num = int(random.random() * num_images)
#    x0, y0 = T[img_num,0]-x_size/2., T[img_num,1]-y_size/2.
#    x1, y1 = T[img_num,0]+x_size/2., T[img_num,1]+y_size/2.
#    img = df.iloc[img_num,:].reshape(num_pixels, num_pixels)
#    ax.imshow(img, aspect='auto', cmap=plt.cm.gray, interpolation='nearest', zorder=100000, extent=(x0, x1, y0, y1))

  # It also plots the full scatter:
ax.scatter(T[:,0],T[:,1],T[:,2], marker='.',alpha=0.7)
