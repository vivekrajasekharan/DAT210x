...
#
# .. your code here ..
df= pd.read_csv("D:\Python_Microscoft\DAT210x\Module5\Datasets\CDR.csv")
...
#
# .. your code here ..
List_peeps=np.ndarray.tolist(df.In.unique())
...
#
# .. your code here ..
user1b=user1.loc[:,["TowerLon", "TowerLat"]]
kmeans=KMeans(n_clusters=2).fit(user1b)
labels=kmeans.predict(user1b)
coordinates=kmeans.cluster_centers_
print(coordinates)
fig = plt.figure()
ax = fig.add_subplot(111)
ax.scatter(coordinates[:,0],coordinates[:,1], c='r', marker='x',linewidth=2, alpha=0.2)
ax.set_title('Kmeans_cluster')
showandtell()  # TODO: Comment this line out when you're ready to proceed
...
# .. your code here ..
All_coords=[]
for dudeX in range (1,10):
    userX=df[df.In==List_peeps[dudeX]]
    userX.plot.scatter(x='TowerLon', y='TowerLat', c='gray', alpha=0.1, title='Call Locations')
    showandtell()  # Comment this line out when you're ready to proceed
    userX=userX[ (userX.DOW=='Sat') | (userX.DOW=='Sun')]
    userX=userX[(userX.CallTime <"06:00:00") | (userX.CallTime >"22:00:00") ]
    fig = plt.figure()
    ax = fig.add_subplot(111)
    ax.scatter(userX.TowerLon,userX.TowerLat, c='g', marker='o', alpha=0.2)
    ax.set_title('Weekend Calls (<6am or >10p)')
    showandtell()  # TODO: Comment this line out when you're ready to proceed
    userXb=userX.loc[:,["TowerLon", "TowerLat"]]
    kmeans=KMeans(n_clusters=2).fit(userXb)
    labels=kmeans.predict(userXb)
    coordinates=kmeans.cluster_centers_
#    print(coordinates)
    fig = plt.figure()
    ax = fig.add_subplot(111)
