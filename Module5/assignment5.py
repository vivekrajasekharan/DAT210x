...
df=pd.read_csv("D:\Python_Microscoft\DAT210x\Module5\Datasets\wheat.data", index_col=0)
#
...
df.drop(labels="wheat_type", axis=1, inplace=True)  
...
for col in df.columns:
    df.loc[:,[col]]=df.loc[:,[col]].fillna(df.loc[:,[col]].mean(), inplace =True)
df.isnull().sum()
#
...
#T_train = preprocessing.Normalizer().fit_transform(data_train)
#
#T_test = preprocessing.Normalizer().fit_transform(data_test)
#
#plt.hist(T_train)
#plt.figure()
#plt.hist(T_test)
...
pca=  PCA(n_components=2).fit_transform(T)
#model = PCA(n_components=2).fit(data_train, label_train)
#pca_train=model.transform(data_train)
#pca_test=model.transform(data_test)
...
X_train, X_test, y_train, y_test = train_test_split (pca, y, test_size = 0.33, random_state =1)

knn=KNeighborsClassifier(n_neighbors=9).fit(X_train, y_train)
#knn.predict(pca_test)
#knn.predict_proba(pca_test)
